const API_BASE_URL = 'http://localhost/blog/backend/filter.php';
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  let category_Title = document.getElementById('category_title')

  const searchParams = new URLSearchParams(window.location.search);
  let container_list = document.getElementById('container_list')


  category_Title.textContent = searchParams.get('name')
  onLoadBuild()
  ...
  async function onLoadBuild() {
    ...
    try {
      const routePromise = await fetch(`${API_BASE_URL}?action=byCategory&category=${searchParams.get('name')}`);
